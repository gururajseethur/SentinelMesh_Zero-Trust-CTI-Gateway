...
try {
  const tab = await waitForJson(`http://127.0.0.1:${port}/json/new?${encodeURIComponent('http://localhost:9000')}`,15000,{method:'PUT'});
  ...
  await send('Page.enable'); await send('Page.navigate',{url:'http://localhost:9000'}); await sleep(6000);
  const shot = await send('Page.captureScreenshot',{format:'png',fromSurface:true});
  await writeFile(`${outputDir}/thehive-live-1440x900.png`, Buffer.from(shot.data,'base64'));
  console.log('Captured TheHive');
  sock.close();
} finally { br.kill(); }
