...
const tab = await waitForJson(`http://127.0.0.1:${port}/json/new?${encodeURIComponent('http://localhost:8180/admin')}`, 15000, { method: 'PUT' });
...
await send('Page.navigate', { url: 'http://localhost:8180/admin' });
await sleep(4000);

const shot = await send('Page.captureScreenshot', { format: 'png', fromSurface: true });
await writeFile(`${outputDir}/keycloak-admin-login-1440x900.png`, Buffer.from(shot.data, 'base64'));
console.log('Captured: keycloak-admin-login-1440x900.png');
