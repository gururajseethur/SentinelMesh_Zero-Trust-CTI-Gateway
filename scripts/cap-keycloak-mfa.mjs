...
try {
  // Navigate to Keycloak admin auth flows page
  const tab = await waitForJson(`http://127.0.0.1:${port}/json/new?${encodeURIComponent('http://localhost:8180/admin/master/console/')}`,15000,{method:'PUT'});
  ...
  await send('Page.navigate',{url:'http://localhost:8180/admin/master/console/'});
  await sleep(5000);
  // Capture the admin console
  const shot = await send('Page.captureScreenshot',{format:'png',fromSurface:true});
  await writeFile(`${outputDir}/keycloak-admin-console-1440x900.png`, Buffer.from(shot.data,'base64'));
  console.log('Captured Keycloak admin console');
  // Now navigate to authentication flows
  await send('Page.navigate',{url:'http://localhost:8180/admin/master/console/#/master/authentication/flows'});
  await sleep(5000);
  const shot2 = await send('Page.captureScreenshot',{format:'png',fromSurface:true});
  await writeFile(`${outputDir}/keycloak-auth-flows-1440x900.png`, Buffer.from(shot2.data,'base64'));
  console.log('Captured Keycloak auth flows');
  sock.close();
} finally { br.kill(); }
